...
### Отложить изменения, а потом вернуть их
```

git status # посмотрели состояние ветки, в которой сейчас находимся
git stash # «спрятали» незакоммиченные изменения в ветке develop 
git checkout main # перешли в main

git stash list # Просмотреть их список
git stash pop # «вынули» последние отложенные изменения
git stash apply stash@{1} # вернули изменения, которые были отложены первыми и находились внизу копилки
